...
# Not re-exported from the whisperx package root -- its __init__ lazily
# exposes only align, assign_word_speakers, load_align_model, load_model,
# load_audio, setup_logging and get_logger. whisperx's own CLI imports it
# from this module too (whisperx/transcribe.py).
...
## Результат приёмки (факт)

Эталонная запись, 4840 с. Транскрипт побайтово совпал с довертикальным:
md5 `c1d06fca712248ef5a48d22f7304a807`.

| Этап | Было | Стало | RTF |
|---|---|---|---|
| декод | — | 8.8 с | 0.002 |
| диаризация | 1ч08м38с | **275.9 с** | 0.057 |
| распознавание | 17м14с | 957.0 с | 0.198 |
| выравнивание | 4м35с | 155.6 с | 0.032 |
| **итого** | **1ч30м58с** | **18м42с** | **0.232** |

Ускорение 4.87×. Последовательная сумма этапов дала бы 1397.5 с против
фактических 1121.6 — то есть 275.9 с диаризации спрятаны целиком. Час записи
проецируется в 13.9 минуты, внутри цели 1:0.25.

## Что разошлось с планом при исполнении

Правки, внесённые по ходу и уже отражённые выше в тексте задач:

- **`whisperx.DiarizationPipeline` не существует.** Пакет реэкспортирует из
  корня только семь имён. Код падал бы с `AttributeError` на каждом реальном
  прогоне; импорт исправлен на `from whisperx.diarize import ...`, как это
  делает и сам CLI whisperx.
- **Пустой массив под bash 3.2.** `"${DIARIZE_ARGS[@]}"` под `set -u` на macOS
  даёт «unbound variable», причём на пути по умолчанию. Заменено на
  `"${DIARIZE_ARGS[@]+"${DIARIZE_ARGS[@]}"}"`.

Добавлено сверх плана по итогам ревью:

- `transcribe()` передаёт `verbose` и `chunk_size` явно — CLI whisperx делает
  это, а без первого из лога пропадал построчный прогресс.
- `worker.join()` ограничен `max(300, длительность аудио)`, поток помечен
  `daemon`, сообщение о таймауте отличается от сообщения об ошибке. Иначе
  зависшая диаризация блокировала бы очередь автоматизации на сутки.
- `StageLog._emit` пишет в stdout одной операцией под блокировкой. При выводе
  в stderr строки таймингов вклеивались в середину буферизованного вывода
  whisperx, и `grep "^whisp:"` их не находил — то есть ровно то, ради чего они
  существуют, не работало в единственном месте, где их читают.
- Блок выравнивания и назначения спикеров обёрнут в `try/except`: до этого
  сбой MPS или скачивания модели после завершения распознавания выбрасывал
  готовый транскрипт.
- Тест на порядок «выравнивание → назначение спикеров». Перестановка этих двух
  вызовов молча снимает разметку спикеров со всех транскриптов, а прежний
  набор тестов оставался при этом полностью зелёным.
- Ожидаемое число потоков CTranslate2 в тесте выводится из установленного
  whisperx, а не зашито литералом.

## После выполнения

- Полный сценарий через автоматизацию прогнан: `./automation/process-new-file.sh`
  на 64-секундном файле, все пять строк `whisp:` в логе привязаны к началу
  строки, транскрипт не изменился, сообщение о саммари говорит «failed»
  вместо ложного «timed out».
